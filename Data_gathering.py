## The following code is used to identify the required weather data for analysisand then gather that information from NOAA
...
# Function to grab weather data from NOAA through online API
...
if __name__ == '__main__':
    ...
    #Initialization to use to activate pull code
    grab_weather = 0
    weather_dblchk = 0

    # reading in the files and appending all data into one dataframe
    df = pd.read_csv('Clean_population_record_for_select_Mammalia.csv')
    df2 = pd.read_csv('Clean_population_record_for_select_Amphibia.csv')
    df3 = pd.read_csv('Clean_population_record_for_select_Reptilia.csv')
    ...
    stations = pd.read_csv('StationListUS.csv')
    ...
    # the following for loop preapres list of stations to grab weather from with all necessary look up information
    ...
    ################# The Code to pull all of the weather data from the API.
    if grab_weather == 1:
    ...
    Weatherdata2 = pd.read_csv('WeatherData2.csv')
    ...
    # Connect the Station ID data to the study id so that it can be merged with pop data in df_clean
    ...
    output_file_name = 'Final2.csv'
    Final.to_csv(output_file_name, index=False)
    
    # Identify any pop data without weather info for second API call in case. Missing info saved as WeatherData2 below.
